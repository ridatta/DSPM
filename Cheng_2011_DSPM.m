...
% Parameters
%%% ACOUSTIC PROPERTIES %%%
f = 1*10^6; % hz, frequency
rho = 998; % kg/m^3, medium density
...
rs = 0.25 * 10^-3; % m, source radius
pS = 2 * rs; % m, source pitch distance
R0 = 0.5 * 12.7* 10^-3; % m, Transducer radius
L0 = 0; % Transducer location, m 
dir0 = [0;0;1]; % transducer direction vector
R_ref = 30 * 10^-3; % Reflector radius, [m]
L_ref = 50 * 10^-3; % reflector location, m 
dir_ref = [0;0;-1]; % reflector direction vector
des_typ = 'rect'; % descretization scheme

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%  CIRCULAR TRANSDUCER GEOMETRY %
[posAT,nrmAT] = descritize(R0,pS,L0,dir0,des_typ); 
posAS = posAT; posAS(:,3) = L0 - rs; % transducer source points
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%% REFLECTOR GEOMETRY %%%
refOn = false; 
if refOn
[posR,nrmR] = descritize(R_ref,pS,L_ref,dir_ref,des_typ); 
posRS = posR; posRS(:,3) = rs * dir_ref(end); % reflector source points
else
    posRS = []; posR = []; nrmR = [];
end
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
[press, fldPts] = getPressureField([-2*R0,2*R0],sort([L0,50*10^-3]),posS,A,k); 
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%% OUTPUT LOG %%%     
fname = ['Cheng-2011-DSPM-Output-' datestr(clock,29)];
save([fname, '.mat']); 
