...
v0 = 1; % m/s, amplitude of surface velocity
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%                  
% Create elements of MM matrix - Transducer
MM_tr = zeros(numTR,numTR); % M matrix
parfor m = 1:numTR
    for n = 1:numTR
        nrm_n = nrmAT(n,:); % normal tp the surface at pt. n
        r_mn = posAT(n,:) - posAS(m,:); % Position vector from source m to n
        ...
        MM_tr(n,m) = 1 / (1i * omega * rho) * n_mn * exp(-1i * k * norm(r_mn)) * ...
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%  
%%%% BOUNDARY CONDITIONS %%%%
V_tr = v0 * ones(numTR,1); % Velocity, m/s
%%% SOLVE FOR ACOUSTIC STRENGTHS %%%%
A_tr = MM_tr \ V_tr; % Acoustic strength of trnsducer
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%  
% Create elements of MM matrix - Reflector
MM_ref = zeros(numR,numR); % M matrix
parfor m = 1:numR
    for n = 1:numR
        nrm_n = nrmR(n,:); % normal tp the surface at pt. n
        r_mn = posR(n,:) - posRS(m,:); % Position vector from source m to n
        n_mn = dot(r_mn'/norm(r_mn), nrm_n'); % Projection of r_mn on normal to surface
        if n <= (numTR + numR) % if target point does not lie on surface of droplet
        MM_ref(n,m) = 1 / (1i * omega * rho) * n_mn * exp(-1i * k * norm(r_mn)) * ...
            (1i * k / norm(r_mn) + 1 / norm(r_mn)^2); % element in row n and column m
        else
        if ~(isRigid)
        MM_ref(n,m) = 1 / (1i * omega * rho) * n_mn * exp(-1i * k * norm(r_mn)) ...
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%  
%%%% BOUNDARY CONDITIONS %%%%
%%% Find velocity at reflector surface due to transducer only
[~,V_tr_r] = getPressureVelocity(posR,nrmR,posAS,A_tr,k,omega,rho);
V_ref = -V_tr_r; % Velocity, m/s
%%% SOLVE FOR ACOUSTIC STRENGTHS %%%%
A_ref = MM_ref \ V_ref; % Acoustic strength
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%% SCALE-FACTOR %%%
...
[~,vel] = getPressureVelocity(posTT,nrmTT,[posAS;posRS],[A_tr;A_ref],k,omega,rho);
...
[press_tr, fldPts] = getPressureField([-L0,L0],sort([L0,L_ref]),[posAS],[A_tr],k); 
[press_r, fldPts] = getPressureField([-L0,L0],sort([L0,L_ref]),[posRS],[A_ref],k); 
press_scaled = scale_factor * (press_tr + press_r);
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%% ANTINODAL PRESSURE %%%
AN = [0,0,L0+dir0(end)*0.5*wl];
[antiNodalPressure,~] = getPressureVelocity(AN,[0,1,0],[posAS;posRS],[A_tr; A_ref],k,omega,rho);
antiNodalPressure = scale_factor * antiNodalPressure; 
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%% ACOUSTIC RADIATION PRESSURES %%%
% Get first-order pressures and velocities
[p1,u1] = getPressureVelocity(posD,nrmD,[posAS;posRS],[A_tr; A_ref],k,omega,rho);
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%% OUTPUT LOG %%%     
fname = ['Wada-2011-DSPM-Output-V3-' datestr(clock,29)];
save([fname, '.mat']); 
